...
public class UserInterface {
     ...
     * @throws SAXException
     */
    public static void main2(String spath, String xpath) throws ParserConfigurationException, IOException, SAXException, WrongInputException {
     ...
     * @throws SAXException
     */
    public void MainMenu(String StammPath, String XMLPath) throws ParserConfigurationException, IOException, SAXException, WrongInputException {
        ...
        String txtpath = "src/main/resources/PRG_WiSe22_220.txt";
        //MongoDBConnectionHandler.protocolInserter(txtpath, allProtocols);
     ...
     * @param fraction
     */
    public void doMenu(int CHOICE, ArrayList<Speaker> sProtocol, ArrayList<Protocol> pProtocol, ArrayList<String> fraction) throws WrongInputException {
        switch (CHOICE) {
            case 1:
     ...
     * @param sSpeaker
     */
    public void allSortedSpeaker(ArrayList<Speaker> sSpeaker) {
     ...
     */
    public void sortedSpeakerAgenda(ArrayList<Protocol> sProtocol, Integer Sitzungsnummer) {
     ...
     * @param pProtocol
     */
    public void Task2Menu(ArrayList<Speaker> sProtocol, ArrayList<Protocol> pProtocol) throws WrongInputException {
        CLOSING = false;
        while(!CLOSING) {
     ...
     * @param pProtocol
     */
    public void mainMenuTask2(int CHOICE, ArrayList<Speaker> sProtocol, ArrayList<Protocol> pProtocol) throws WrongInputException {
        switch (CHOICE){
            case 1:
                protocolinformation(pProtocol); //gets all information of every protocol
                break;
            case 2:
                System.out.println("When you enter somthing less 0 it will be corrected to 1");
                System.out.println("When you enter somthing 240 it will be corrected to 239");
            ...
            case 5:
                ...
                break;
            case 6:
                MpSessionCounter(pProtocol); //How often a MP has lead a session
                break;
            case 7:
                CLOSING = true;
                break;
    ...
    public void Task3Menu(ArrayList<Speaker> sProtocol, ArrayList<Protocol> pProtocol, ArrayList<String> fraction) {
        CLOSED = false;
        while(!CLOSED) {
            System.out.println("\nBelow you will see some actions which you can perform");
    ...
    public void mainMenuTask3(int CHOICE, ArrayList<Speaker> sProtocol, ArrayList<Protocol> pProtocol, ArrayList<String> fraction){
        switch (CHOICE){
            case 1:
            ...
            case 2:
                ...
                Speakers = getSpeakerAverageLength(pProtocol, sProtocol); // gets the average Speech length of every Speaker
                break;
            case 3:
            ...
            case 7:
                ...
                break;
            case 8:
                averageCommentsFraction(pProtocol,fraction); // get average Comments each speaker receives from each fraction
                break;
            case 9:
                CLOSED = true;
                break;
     ...
     * @param pProtocol
     */
    public static void MpSessionCounter(ArrayList<Protocol> pProtocol) {
     ...
     * @param sProtocol
     */
    public static ArrayList<Speaker> getSpeakerAverageLength(ArrayList<Protocol> pProtocol, ArrayList<Speaker> sProtocol){
        ...
        for(int m = 0; m < sProtocol.size(); m++) {
            ...
            sProtocol.get(m).setAmountSpeeches(sCount);
            sProtocol.get(m).setaverageSpeechLength(((sLength - sSpeaker)/sCount));
        }
        return sProtocol;
    ...
    public static void getLongestSpeech(ArrayList<Protocol> pProtocol) {
            ...
            for (int j = 0; j < pProtocol.get(i).getTagesOrdnungsPunkte().size(); j++) { //through every agenda
                for (int k = 0; k < pProtocol.get(i).getTagesOrdnungsPunkte().get(j).getSpeeches().size(); k++) { //through every speech of an agenda
                    if (pProtocol.get(i).getTagesOrdnungsPunkte().get(j).getSpeeches().get(k).getText().length() > sLength) { // we check if the previous declared longest speech is being toped
                        ...
                        ID = pProtocol.get(i).getTagesOrdnungsPunkte().get(j).getSpeeches().get(k).getSpeechID();
        ...
        System.out.println("She spoke: " + sLength + " words and the ID of the Speech is: " + ID);
    }
    ...
    public static void getshortestSpeech(ArrayList<Protocol> pProtocol) {
        ...
        ID = pProtocol.get(a).getTagesOrdnungsPunkte().get(b).getSpeeches().get(c).getSpeechID();
        ...
        System.out.println("He spoke: " + (sLength - subNameLength) + " words and the SpeechID is: " + ID);
    ...
    public static void topCommentsList(ArrayList<Protocol> pProtocol, int limit) {
        ...
        ArrayList<Date> Date = new ArrayList<>(); // ArrayList with all the dates of the speech
    ...
    public static void averageCommentsFraction(ArrayList<Protocol> pProtocol, ArrayList<String> fraction){
        for (int i = 0; i < fraction.size(); i++) { //We iterate through every fraction
            ...
            System.out.println("The Fraction: " + fraction.get(i) + " recived: " + amountComments + " comments");
            System.out.println("In average every Speaker receives the following amount of comments of each fraction -> " + " CDU/CSU: " + (CDUCSU/amountspeeches) + " SPD: " + (SPD/amountspeeches) + " AfD: " + (Afd/amountspeeches) + " BÜNDNIS 90/DIE GRÜNEN: " + (Bündnis/amountspeeches) + " DIE LINKE: " + (Linke/amountspeeches) + " FDP: " + (FDP/amountspeeches) + " Beifall: " + Beifall);
            System.out.println("\n");
        }
    }
