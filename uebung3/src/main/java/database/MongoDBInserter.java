...
public class MongoDBInserter {
    ...
    public static void SpeechInserter(String xPath, ArrayList<Speech> sSpeech, MongoCollection collection) throws IOException {
        ArrayList<Document> documentSpeech = new ArrayList<>(); //create one big ArrayList with documents
        for(int x = 0; x < sSpeech.size(); x++){
            Document docSpeech = new Document();
            Speech_File_Impl speech = (Speech_File_Impl) sSpeech.get(x);
            docSpeech.put("_id", speech.getSpeechID());
            docSpeech.put("content", speech.getText());
            docSpeech.put("speakerid", speech.getID());
            docSpeech.put("sitzungsnr", speech.getSitzungsnr());
            for(int y = 0; y < sSpeech.get(x).getSpeaker().size(); y++) {
                Document docSpeaker = new Document();
                String speakerFirstName = speech.getSpeaker().get(y).getFirstName(); // get the speakers name
                String speakerLastName = speech.getSpeaker().get(y).getLastName(); // get the speakers name
                String speakerID = speech.getSpeaker().get(y).getID(); // get the speakers id
                String speakerFraction = speech.getSpeaker().get(y).getFraction(); // get the speakers fraction
                String speakerParty = speech.getSpeaker().get(y).getParty();
                docSpeaker.put("_id", speakerID);
                docSpeaker.put("firstname", speakerFirstName);
                docSpeaker.put("lastname", speakerLastName);
                docSpeaker.put("fraction", speakerFraction);
            ...
            List<Document> docComments = new ArrayList<>();
            for (int j = 0; j < sSpeech.get(x).getComments().size(); j++) { // for every comment of one speech
                Comment_File_Impl comment = (Comment_File_Impl) sSpeech.get(x).getComments().get(j); //we create a new comment object
                Document docComment = new Document();
                String commentID = speech.getSpeechID(); // comment id = speech id
                String commentContent = comment.getContent(); // content of the commen
                docComment.put("_id", commentID);
            ...
            docSpeech.append("comments", docComments); //add comments to the speech
            documentSpeech.add(docSpeech);


            }
        MongoDBConnectionHandler.dbInserter(xPath, documentSpeech, "speech"); // insert the protocols to the db
